...
const TVShows = () => {
  ...
  const [initialLoading, setInitialLoading] = useState(true); // Control the loader visibility
  const [isContentVisible, setIsContentVisible] = useState(false); // Control content visibility
  const [hasMounted, setHasMounted] = useState(false); // Track initial mount state

  const { tvShows, totalPages, fetchTrailerForTvShow, fetchLatestTVShows } =
    useMovies();
  const router = useRouter();

  // Ensure component is fully mounted before making any state changes
  useLayoutEffect(() => {
    setHasMounted(true);
  }, []);

  // Load TV shows only after component has mounted
  useEffect(() => {
    if (!hasMounted) return; // Skip loading until the component is mounted

    const loadShows = async () => {
      try {
        setInitialLoading(true);
        await fetchLatestTVShows(page);
      } catch (error) {
        console.error("Error loading TV shows:", error);
      } finally {
        setInitialLoading(false);
        setIsContentVisible(true); // Show content after data is fetched and loading completes
      }
    };

    loadShows();
  }, [page, fetchLatestTVShows, hasMounted]);
  ...
  const handleMouseEnter = (tvShow) => {
    setHoveredTvShow(tvShow);
  ...
  const fetchTvShowDetails = async (tmdbId) => {
    try {
      const response = await axios.get(
        `http://localhost:3001/api/sonar/tvshows/${tmdbId}`
      );
  ...
  const handlePlayTrailer = async (tmdbId) => {
    const trailerId = await fetchTrailerForTvShow(tmdbId);
    if (trailerId) {
  ...
  // Render Loader if the component is still fetching data
  ...
  return (
    <div className="w-screen h-screen">
      ...
      >
        ...
        <div className="pt-44 w-full h-24 p-4 flex justify-center items-center">
          <h1 className={`text-white text-6xl text-center ${Poiret.className}`}>
            Latest TV Shows
          </h1>
        </div>
        <br />
        <hr className="h-1 bg-gradient-to-r from-red-700 via-green-500 to-black border-0 my-4" />
              ...
              >
                ...
                {hoveredTvShow?.id === tvShow.id && tvShowDetails && (
                  <div className="absolute inset-0 bg-black bg-opacity-80 p-4 flex flex-col">
                    ...
                    <button
                      onClick={() => handlePlayTrailer(tvShow.id)}
...
//       }, 200); // Similar delay as in Portal component for smooth transition
//     };

//     loadShows();
//   }, [page, fetchLatestTVShows]);
